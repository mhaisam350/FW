...
export const CarItem = ({ children, width, background }) => {
    return (
        <article className={styles['carousel-item']} style={ { width: width, backgroundImage: `url(${background})` }  } > 
            { children }
        </article>
    )
}
...
const MainCarousel = ({ children }) => {
    ...
    const nextSlide = () => {
        ...
        // setCurrent(current === slides - 1 ? current : current + 1);
    }
    ...
    return (
        ...
        <main className={styles.carousel} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} ref={sliderContainerRef}>
            ...
            <div className={styles['inner'] + " " + transitionClass} onPointerDown={handlePointerStart} onPointerMove={handlePointerMove} onPointerUp={handlePointerEnd} onPointerCancel={handlePointerCancel} style={ { transform: `translateX(${-(current * 100) + swipeDistancePercentage()}%)`} }>
            ...
            <div className={styles.indicators}>
                ...
                {React.Children.map(children, (child, index) => {
                    return (
                        <button onClick={() => setCurrent(index)} className={`${index === current ? styles.active : ""}`}> {index + 1} </button>
                    )
                })}
